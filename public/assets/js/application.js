...
function handlePanelAction() {
    ...
    function handlePanelControls() {
        ...
        $('.panel-controls-min').each(function() {
            var controls_html = '<div class="control-btn">' + '<a href="#" class="panel-toggle-min"><i class="fas fa-angle-down termOpen" id="termOpen"></i><i class="fas fa-angle-up " id="termClose" style="display: none;"></i></a>' + '</div>';
            $(this).append(controls_html);
        });

        $('.panel-controls-min-header').each(function() {
            var controls_html = '<div class="control-btn">' + '<a href="#" class="panel-toggle-min-header"><i class="fas fa-angle-up termOpen icon" id="termOpen"></i><i class="fas fa-angle-down icon" id="termClose" style="display: none;"></i></a>' + '</div>';
            $(this).append(controls_html);
        });

        $('.panel-controls-min-show').each(function() {
            var controls_html = '<div class="control-btn">' + '<a href="#" class="panel-toggle-min-show"><i class="fas fa-angle-up termOpen icon" id="termOpen" style="display: none;"></i><i class="fas fa-angle-down icon" id="termClose"></i></a>' + '</div>';
            $(this).append(controls_html);
        });
    }
    handlePanelControls();
    ...
    $(document).on("click", ".panel-header .panel-toggle-min", function(event) {
        event.preventDefault();
        $(this).toggleClass("closed").parents(".panel:first").find(".panel-content").slideToggle();
        $(this).find(".svg-inline--fa").slideToggle();
        setTimeout(function(){
          fixedScreen()
        }, 250);
    });
    ...
    $(document).on("click", ".panel-header .panel-toggle-min-header", function(event) {
        event.preventDefault();
    });
    // Popout / Popin Panel
...
/*
    ...
    define rotate function for rotating arrow icon of parent menu 
*/
...
function toggleSidebarMenu() {
    ...
    $('.sidebar').on('click', '.nav-sidebar li.nav-parent > a', function(e) {
        ...
        if (sub.is(":visible")) {
            ...
            //alert('tutup');
            $(this).find('.svg-inline--fa.arrow').rotate(0); // adly add
        } else {
            ...
            //alert('buka');
            $('li.nav-parent').find('.svg-inline--fa.arrow').rotate(0); // adly add
            $(this).find('.svg-inline--fa.arrow').rotate(180); // adly add
        }
    });
}
